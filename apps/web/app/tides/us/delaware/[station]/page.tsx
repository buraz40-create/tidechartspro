...
import { getStationBySlug, getAllSlugs, DELAWARE_STATIONS } from '@/lib/delaware-stations'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://tidechartspro.com/tides/us/delaware/${slug}`
  return {
    title, description,
    keywords: [`${city.toLowerCase()} tide chart`, `${city.toLowerCase()} tides for fishing`, `${city.toLowerCase()} fishing tides`, 'delaware tide chart', 'tides for fishing'],
    alternates: { canonical: url },
    openGraph: { type: 'website', url, title, description, siteName: 'TideChartsPro', images: [{ url: `https://tidechartspro.com/og/tides/delaware/${slug}.png`, width: 1200, height: 630 }] },
...
export default async function StationPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <TideLocationPage station={{ ...station!, nearby: computeNearby(station!, DELAWARE_STATIONS) }} />
    </>
  )
}
