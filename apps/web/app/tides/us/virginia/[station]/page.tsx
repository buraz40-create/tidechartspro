...
import { getStationBySlug, getAllSlugs, VIRGINIA_STATIONS } from '@/lib/virginia-stations'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://tidechartspro.com/tides/us/virginia/${slug}`
  return {
    title, description,
    keywords: [`${city.toLowerCase()} tide chart`, `${city.toLowerCase()} tides for fishing`, `${city.toLowerCase()} fishing tides`, 'virginia tide chart', 'tides for fishing'],
    alternates: { canonical: url },
    openGraph: { type: 'website', url, title, description, siteName: 'TideChartsPro', images: [{ url: `https://tidechartspro.com/og/tides/virginia/${slug}.png`, width: 1200, height: 630 }] },
...
export default async function StationPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <TideLocationPage station={{ ...station!, nearby: computeNearby(station!, VIRGINIA_STATIONS) }} />
    </>
  )
}
