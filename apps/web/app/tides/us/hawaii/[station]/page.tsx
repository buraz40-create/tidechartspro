...
import { getStationBySlug, getAllSlugs, HAWAII_STATIONS } from '@/lib/hawaii-stations'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://tidechartspro.com/tides/us/hawaii/${slug}`
  return {
    title, description,
    keywords: [`${city.toLowerCase()} tide chart`, `${city.toLowerCase()} tides for fishing`, `${city.toLowerCase()} fishing tides`, 'hawaii tide chart', 'tides for fishing'],
    alternates: { canonical: url },
    openGraph: { type: 'website', url, title, description, siteName: 'TideChartsPro', images: [{ url: `https://tidechartspro.com/og/tides/hawaii/${slug}.png`, width: 1200, height: 630 }] },
    ...
    '@graph': [
      { '@type': 'WebPage', '@id': url, url, name: `${city} Tide Chart | Tides for Fishing`,
        ...
        breadcrumb: { '@type': 'BreadcrumbList', itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Hawaii', item: 'https://tidechartspro.com/tides/us/hawaii' },
...
export default async function StationPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <TideLocationPage station={{ ...station!, nearby: computeNearby(station!, HAWAII_STATIONS) }} />
    </>
  )
}
