...
import { getStationBySlug, getAllSlugs, MISSISSIPPI_STATIONS } from '@/lib/mississippi-stations'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const city = s.city.replace(/, MS$/, '')
  const cityMS = s.city

  const title = `${city} Tide Chart | Tides for Fishing | TideChartsPro`
  const description =
    `${cityMS} tide chart with live tide levels, tides for fishing, solunar periods ` +
    `and hourly fishing forecast. ${city} tide level chart updated every 6 minutes. ` +
    `Best fishing tides for ${city} today.`

  const url = `https://tidechartspro.com/tides/us/mississippi/${slug}`
  const ogImage = `https://tidechartspro.com/og/tides/mississippi/${slug}.png`
  ...
  return {
    ...
    keywords: [
      ...
      `${city.toLowerCase()} MS tide chart`,
      `mississippi tide chart`,
      `mississippi tides for fishing`,
      ...
      },
      {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            acceptedAnswer: { '@type': 'Answer', text: `The mean tidal range in ${city} is approximately ${s.meanRange} feet. Gulf Coast tides are diurnal — predominantly one high and one low tide per day.` },
          },
        ],
      },
    ],
...
export default async function StationPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <TideLocationPage station={{ ...station!, nearby: computeNearby(station!, MISSISSIPPI_STATIONS) }} />
    </>
  )
}
