...
import { getStationBySlug, getAllSlugs, LOUISIANA_STATIONS } from '@/lib/louisiana-stations'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const city = s.city.replace(/, LA$/, '')
  const cityLA = s.city

  const title = `${city} Tide Chart | Tides for Fishing | TideChartsPro`
  const description =
    `${cityLA} tide chart with live tide levels, tides for fishing, solunar periods ` +
    `and hourly fishing forecast. ${city} tide level chart updated every 6 minutes. ` +
    `Best fishing tides for ${city} today.`

  const url = `https://tidechartspro.com/tides/us/louisiana/${slug}`
  const ogImage = `https://tidechartspro.com/og/tides/louisiana/${slug}.png`
  ...
  return {
    ...
    keywords: [
      ...
      `${city.toLowerCase()} LA tide chart`,
      `louisiana tide chart`,
    ...
    '@graph': [
      {
        ...
        breadcrumb: {
          ...
          itemListElement: [
            ...
            { '@type': 'ListItem', position: 3, name: 'Louisiana', item: 'https://tidechartspro.com/tides/us/louisiana' },
      ...
      },
      {
        ...
        mainEntity: [
          ...
          },
          {
            ...
            acceptedAnswer: { '@type': 'Answer', text: `The mean tidal range in ${city} is approximately ${s.meanRange} feet. Louisiana Gulf Coast tides are diurnal — predominantly one high and one low tide per day, influenced by wind and atmospheric pressure.` },
          },
        ],
      },
    ],
...
export default async function StationPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <TideLocationPage station={{ ...station!, nearby: computeNearby(station!, LOUISIANA_STATIONS) }} />
    </>
  )
}
