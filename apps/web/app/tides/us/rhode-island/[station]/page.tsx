...
import { getStationBySlug, getAllSlugs, RHODE_ISLAND_STATIONS } from '@/lib/rhode-island-stations'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `https://tidechartspro.com/tides/us/rhode-island/${slug}`
  return {
    title, description,
    keywords: [`${city.toLowerCase()} tide chart`, `${city.toLowerCase()} tides for fishing`, `${city.toLowerCase()} fishing tides`, 'rhode island tide chart', 'tides for fishing'],
    alternates: { canonical: url },
    openGraph: { type: 'website', url, title, description, siteName: 'TideChartsPro', images: [{ url: `https://tidechartspro.com/og/tides/rhode-island/${slug}.png`, width: 1200, height: 630 }] },
    ...
    '@graph': [
      { '@type': 'WebPage', '@id': url, url, name: `${city} Tide Chart | Tides for Fishing`,
        ...
        breadcrumb: { '@type': 'BreadcrumbList', itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Rhode Island', item: 'https://tidechartspro.com/tides/us/rhode-island' },
...
export default async function StationPage({ params }: Props) {
  ...
  return (
    <>
      ...
      <TideLocationPage station={{ ...station!, nearby: computeNearby(station!, RHODE_ISLAND_STATIONS) }} />
    </>
  )
}
